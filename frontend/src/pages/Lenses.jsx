...
import { RiCameraLensFill, RiCameraLensAiFill } from "react-icons/ri";
...
function Lenses() {
  ...
  const categories = [
    { id: "all", name: "All Categories", icon: <RiCameraLensFill /> },
    { id: "cinematic", name: "Cinematic", icon: <RiCameraLensFill /> },
    { id: "dslr", name: "DSLR", icon: <RiCameraLensFill /> },
    { id: "mirrorless", name: "Mirrorless", icon: <RiCameraLensFill /> },
  ];

  const brands = [
    { id: "all", name: "All Brands", icon: <RiCameraLensAiFill /> },
    { id: "canon", name: "Canon", icon: <RiCameraLensAiFill /> },
    { id: "fujifilm", name: "Fujifilm", icon: <RiCameraLensAiFill /> },
    { id: "nikon", name: "Nikon", icon: <RiCameraLensAiFill /> },
    { id: "panasonic", name: "Panasonic", icon: <RiCameraLensAiFill /> },
    { id: "sigma", name: "Sigma", icon: <RiCameraLensAiFill /> },
    { id: "sony", name: "Sony", icon: <RiCameraLensAiFill /> },
    { id: "tamron", name: "Tamron", icon: <RiCameraLensAiFill /> },
  ];

  useEffect(() => {
    axios
      .get("http://localhost:8000/api/lenses")
      .then((response) => {
  ...
  const getImagePath = (lens) => {
    const brand = lens.model.split(" ")[0].toLowerCase();
    const folder = `${brand}Lens`;
    const fileName = lens.photo.split("\\").pop();
  ...
  const filteredLenses = lenses.filter((lens) => {
    const isInCategory =
      ...
      lens.type.toLowerCase() === selectedCategory;
    const isInPriceRange =
      lens.price >= priceRange[0] && lens.price <= priceRange[1];
    const isInBrand =
      selectedBrand === "all" ||
      lens.model.toLowerCase().includes(selectedBrand.toLowerCase());

    return isInCategory && isInPriceRange && isInBrand;
  });

  const ProductCard = ({ lens }) => (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between mb-3">
            <span className="text-3xl font-bold text-orange-500">
              {lens.price} <span className="text-xl">€</span>
            </span>
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="hidden md:block w-72 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md">
              <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2">
                <RiCameraLensFill className="text-orange-500" />
                Brands
              </h2>
              <div className="space-y-2">
                {brands.map((brand) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredLenses.length > 0 ? (
                filteredLenses.map((lens) => (
                  <ProductCard key={lens.id} lens={lens} />
                ))
              ) : (
                <div className="col-span-full text-center py-12">
                  <div className="text-gray-400 text-6xl mb-4">
                    <RiCameraLensFill className="inline-block" />
                  </div>
