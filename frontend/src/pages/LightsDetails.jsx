...
import { FaArrowLeft, FaLightbulb, FaTimes } from "react-icons/fa";

function LightsDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8000/api/Lights/${id}`)
      .then((response) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <FaLightbulb className="animate-bounce text-4xl text-orange-500 mx-auto mb-4" />
  ...
  const specifications = JSON.parse(light.specifications);
